...
/*   Updated: 2024/10/12 22:30:14 by sanghhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void set_step_and_side_distance_component(double *raydir, int *step, double *side_dist, double player_pos, int map, double delta_dist);
...
static void draw_column(t_game *game, int i, int draw_start, int draw_end, t_ray *ray, int texture_index, double step, double tex_pos);
...
// Draw the background (ceiling and floor) of the game window
static void draw_background(t_game *game)
{
    int y;
    int x;
...
// Initialize ray properties for DDA (Digital Differential Analysis)
static void initialize_ray(t_game *game, t_ray *ray)
{
...
static void set_step_and_side_distance(t_game *game, t_ray *ray)
{
    set_step_and_side_distance_component(&ray->raydir_x, &ray->step_x, &ray->side_dist_x, game->player.pos.x, ray->map_x, ray->delta_dist_x);
    set_step_and_side_distance_component(&ray->raydir_y, &ray->step_y, &ray->side_dist_y, game->player.pos.y, ray->map_y, ray->delta_dist_y);
}
...
// Perform the DDA algorithm to find where the ray hits a wall
static void perform_dda(t_game *game, t_ray *ray)
{
...
// Calculate texture position for the wall being drawn
static void calculate_texture(t_ray *ray, t_game *game)
{
    if (ray->side == 0)
...
// Determine the texture index based on the ray direction and side
static int determine_texture_index(t_ray *ray)
{
    if (ray->side == 0)
    {
...
// Draw a vertical column of the wall with the correct texture
static void draw_column(t_game *game, int i, int draw_start, int draw_end, t_ray *ray, int texture_index, double step, double tex_pos)
{
    int j;
    int tex_y;
